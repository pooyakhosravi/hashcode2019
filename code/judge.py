    ...
    for i in range(1,n):
        ...
        try:
            ...
            for word in t[0]:
                if word in t[1]:
                    scores[-1]+=1
            scores[-1] = min(scores[-1],len(t[0])-scores[-1], len(t[1])-scores[-1])
            
        except ValueError:
            print("Non-integer found in output line at line " + str(i+3) + ": " + line)
            scores.append(0)
            a = []
        except IOError:
...
def main():
    if len(sys.argv) >= 4:
        ...
        if sys.argv[1] == '-f' or sys.argv[1] == '-vf' or sys.argv[1] == '-fv':
            ...
            if verbose:
                print("{0:11} {1:7} {2:<9} {3:>14}".format('Outfile', 'Score', 'Best File', 'Best Score'))
                print("-"*50)
            ...
            for out_file in out_files:
                ...
                if verbose:
                    print("{0:9} {1:^9d} {2:9} {3:9d}".format(out_file.name, score, max_output_name, max_score))
